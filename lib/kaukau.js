...
function Kaukau(options) {
  ...
  // for Kaukau
  let dir = options.directory || 'tests';
  let files = options.files || [];
  let enableLogs =
    typeof options.enableLogs === 'undefined' ? true : options.enableLogs;
  let exitOnFail = options.exitOnFail || false;

  // for Kaukau._parameters
  let parameters = options.parameters;

  // for Mocha
  let mochaOptions = options.options || {};
...
Kaukau.prototype.run = function run(fn) {
  ...
  function runMocha(callback) {
    ...
    let optionsPlus = c.get('mochaOptions') || {};
    let kaukauOptions = c.get('kaukauOptions') || {};
    kaukauOptions.files = kaukauOptions.files || self.getFiles() || [];
    ...
    let options = JSON.parse(JSON.stringify(self.getMochaOptions()));
