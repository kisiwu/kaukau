...
const DEFAULT_OPTIONS = {
  ...
  [KAUKAU_OPTIONS.EXT.name]: ['.js', '.mjs', '.cjs'],
};
...
function _parseKaukauOption(b, opts, defaultOpts, kOpt) {
  ...
  if (typeof opts[k] !== 'undefined') {
    ...
    if (type === 'boolean') {
      ...
      if (Array.isArray(b[k]) && kOpt.elemType) {
        let tmp = b[k].filter(el => typeof el == kOpt.elemType);
        if (!tmp.length && typeof defaultOpts[k] !== 'undefined') {
          b[k] = defaultOpts[k];
        } else {
          b[k] = tmp;
        }
